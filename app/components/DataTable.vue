          ...
          >
            <div class="flex items-center gap-3 flex-1">
              ...
              <div class="flex-1">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ formatTransactionsDate(transaction.time) }}</p>
              </div>
            </div>
            <div class="text-right">
              <p :class="['text-sm font-semibold', transaction.amount > 0 ? 'text-green-600' : 'text-red-600']">
                {{ transaction.amount > 0 ? '+' : '-' }}{{ Math.abs(transaction.amount).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 1 }) }} 
              </p>
            </div>
          </div>
        </div>
